...
{
    return (ch < 0x80) && (isalnum(static_cast<unsigned char>(ch)) || ch == '_');
}

static inline bool IsAWordStart(const int ch)
{
    return (ch < 0x80) && (isalnum(static_cast<unsigned char>(ch)) || ch == '_' || ch == '@' || ch == '#' || ch == '$' || ch == '.');
}

static inline bool IsAOperator(char ch) {
	if (IsASCII(ch) && isalnum(static_cast<unsigned char>(ch)))
		return false;
	if (ch == '+' || ch == '-' || ch == '*' || ch == '/' ||
	    ch == '&' || ch == '^' || ch == '=' || ch == '<' || ch == '>' ||
	    ch == '(' || ch == ')' || ch == '[' || ch == ']' || ch == ',' )
	...
	while ( ( (cTemp = szLine[nPos]) != '\0'))
	{
		...
		if (nStartFound == 1) {
			...
			else if ((nFlag == 1) && (cTemp != '}'))
			{
				...
				if (isdigit(static_cast<unsigned char>(cTemp)) == false) {nSpecNum = 0;}
			}
		}
...
static bool IsContinuationLine(Sci_PositionU szLine, Accessor &styler)
{
	...
	while (nsPos < nePos)
	{
		...
		if (!(stylech == SCE_AU3_COMMENT)) {
			...
			if (!isspacechar(static_cast<unsigned char>(ch))) {
				if (ch == '_')
					return true;
				else
					return false;
